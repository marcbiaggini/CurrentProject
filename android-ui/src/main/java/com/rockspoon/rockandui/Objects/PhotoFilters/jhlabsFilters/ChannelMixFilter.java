...
public class ChannelMixFilter extends PointFilter {
  ...
  public int filterRGB(int x, int y, int rgb) {
    ...
    int g = (rgb >> 8) & 0xff;
    int b = rgb & 0xff;
    int nr = PixelUtil.clamp((intoR * (blueGreen * g + (255 - blueGreen) * b) / 255 + (255 - intoR) * r) / 255);
    int ng = PixelUtil.clamp((intoG * (redBlue * b + (255 - redBlue) * r) / 255 + (255 - intoG) * g) / 255);
    int nb = PixelUtil.clamp((intoB * (greenRed * r + (255 - greenRed) * g) / 255 + (255 - intoB) * b) / 255);
