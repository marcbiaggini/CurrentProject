...
public class FieldWarpFilter extends TransformFilter {
  ...
  public int[] filter(int[] src, int w, int h) {
    if (inLines != null && outLines != null) {
      ...
      for (int line = 0; line < inLines.length; line++) {
        Line l = intermediateLines[line] = new Line(ImageMath.lerp(amount, inLines[line].x1,
            outLines[line].x1), ImageMath.lerp(amount, inLines[line].y1, outLines[line].y1),
            ImageMath.lerp(amount, inLines[line].x2, outLines[line].x2), ImageMath.lerp(amount,
            inLines[line].y2, outLines[line].y2));
        l.setup();
        inLines[line].setup();
      }
