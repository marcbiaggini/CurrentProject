...
public class Gradient extends ArrayColorMap implements Cloneable {
        ...
        if (j >= 0 && j <= 255) {
          ...
          switch (type) {
            ...
            case HUE_CCW:
              ...
              float[] hsv = {ImageMath.lerp(t, hsb1[0], hsb2[0]) % (ImageMath.TWO_PI), ImageMath.lerp(t, hsb1[1], hsb2[1]), ImageMath.lerp(t, hsb1[2], hsb2[2])};
              /*
  ...
  public void mutate(float amount) {
    for (int i = 0; i < numKnots; i++) {
      ...
      int g = ((rgb >> 8) & 0xff);
      int b = (rgb & 0xff);
      r = PixelUtil.clamp((int) (r + amount * 255 * (Math.random() - 0.5)));
      g = PixelUtil.clamp((int) (g + amount * 255 * (Math.random() - 0.5)));
      b = PixelUtil.clamp((int) (b + amount * 255 * (Math.random() - 0.5)));
