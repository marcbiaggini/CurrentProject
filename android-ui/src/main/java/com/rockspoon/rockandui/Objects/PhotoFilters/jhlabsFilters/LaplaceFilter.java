...
public class LaplaceFilter {
  ...
  public int[] filter(int[] src, int w, int h) {
    ...
    for (int y = 0; y < h; y++) {
      if (y < h - 1) {
        row3 = getRGB(src, y + 1, w);
        brightness(row3);
      }

      pixels[0] = pixels[w - 1] = 0xff000000;//FIXME
      ...
      for (int x = 1; x < w - 1; x++) {
        ...
        int r = ((row1[x - 1] + row1[x] + row1[x + 1] +
            ...
            row3[x - 1] + row3[x] + row3[x + 1]) > 0) ? gradient : (128 + gradient);
        pixels[x] = r;
      }

      setRGB(dst, y, w, pixels);
      int[] t = row1;
      row1 = row2;
      row2 = row3;
      row3 = t;
