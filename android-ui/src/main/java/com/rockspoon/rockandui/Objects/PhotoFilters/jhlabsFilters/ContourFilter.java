...
public class ContourFilter extends WholeImageFilter {
  ...
  protected int[] filterPixels(int width, int height, int[] inPixels, Rect transformedSpace) {
    ...
    for (int i = 0; i < 256; i++) {
      table[i] = (short) PixelUtil.clamp((int) (255 * Math.floor(levels * (i + offsetl) / 256) / (levels - 1) - offsetl));
    }

    for (int x = 0; x < width; x++) {
      int rgb = inPixels[x];
      r[1][x] = (short) PixelUtil.brightness(rgb);
    }
    ...
    for (int y = 0; y < height; y++) {
      ...
      if (y < height - 1) {
        for (int x = 0; x < width; x++) {
          ...
          r[2][x] = (short) PixelUtil.brightness(rgb);
        }
      }
      ...
      for (int x = 0; x < width; x++) {
        ...
        if (v != 0) {
          outPixels[index] = PixelUtil.combinePixels(inPixels[index], contourColor, PixelUtil.NORMAL, v);
        } else {
