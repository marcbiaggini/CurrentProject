...
import com.rockspoon.rockandui.Objects.PhotoFilters.JhlabsFilters.Support.Histogram;
...
public class LevelsFilter extends WholeImageFilter {
  ...
  protected int[] filterPixels(int width, int height, int[] inPixels, Rect transformedSpace) {
    ...
    if (histogram.getNumSamples() > 0) {
      ...
      for (i = 0; i < 3; i++) {
        for (j = 0; j < 256; j++) {
          lut[i][j] = PixelUtil.clamp((int) (255 * (lowOutputLevel + (highOutputLevel - lowOutputLevel) * (j - low) / (high - low))));
        }
      }
    } else {
      lut = null;
